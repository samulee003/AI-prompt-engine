...
if (API_KEY) {
  ...
  console.warn('Gemini API Key (process.env.API_KEY) is not defined. API calls will fail.');
}
...
export async function refineUserPrompt(
  ...
  if (!API_KEY || !ai) {
    // This error message will be shown to the user if the API key is not set.
    const errorMsg =
      userLanguage === 'zh-TW'
  ...
  if (!originalPrompt || !originalPrompt.trim()) {
    // This case should be handled by UI validation, but as a safeguard:
    const errorMsg =
