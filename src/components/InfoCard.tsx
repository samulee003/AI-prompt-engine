...
const HeroIcon = ({ name, className = 'w-6 h-6' }: HeroIconProps): React.ReactNode => {
  ...
  // Ensure a valid ReactNode is returned; if name is undefined or not in map, fall back to AcademicCapIcon or null/empty fragment
  const iconSvg = name && iconMap[name] ? iconMap[name] : iconMap['AcademicCapIcon'];
  return iconSvg || null; // Return null if for some reason AcademicCapIcon was also missing
};
...
export const InfoCard = ({ title, children, iconName, onClick }: InfoCardProps): JSX.Element => {
